...
SensorData Sdata = {0, 0.0};
//uint8_t RxData;		//定义串口接收的数据变量
uint8_t Serial_RxFlag;		//定义串口接收的标志位变量
//uint8_t Serial_DrvFlag;	  //数据获取后解析成功
/**
  * 函    数：串口初始化
  * 参    数：无
  * 返 回 值：无
...
//uint8_t Serial_GetDrvFlag(void)
//{
//	if (Serial_DrvFlag == 1)			//如果标志位为1
//	{
//		Serial_DrvFlag = 0;
//		return 1;					//则返回1，并自动清零标志位
//	}
